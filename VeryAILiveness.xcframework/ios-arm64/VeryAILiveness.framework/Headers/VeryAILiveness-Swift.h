...
#ifndef VERYAILIVENESS_SWIFT_H
#define VERYAILIVENESS_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="VeryAILiveness",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
SWIFT_CLASS("_TtC14VeryAILiveness17CameraPreviewView")
...
@interface VeryAILiveness : NSObject
/// Run a liveness check.
/// \param viewController The view controller to present from.
///
/// \param config Optional configuration — theme / language / debug logging.
///
...
///
+ (void)checkFrom:(UIViewController * _Nonnull)viewController config:(VeryLivenessConfig * _Nonnull)config presentationStyle:(enum VeryPresentationStyle)presentationStyle completion:(void (^ _Nonnull)(VeryResult * _Nonnull))completion;
/// Returns whether the device can support a palm scan (camera + RAM).
...
SWIFT_CLASS("_TtC14VeryAILiveness10VeryConfig")
...
/// customizable.
SWIFT_CLASS("_TtC14VeryAILiveness16VeryCustomString")
...
/// Caller-supplied overrides for user-facing status strings, keyed by the
/// stable identifiers on <code>VeryCustomString</code>. Mirrors <code>VeryConfig.customStrings</code>
/// — the liveness page reuses the same scan UI, so the same keys apply. A
/// non-empty value takes priority over the SDK’s built-in localized string;
/// unknown keys and blank values fall back to the localized default. Optional;
/// set via property assignment after init.
@property (nonatomic, copy) NSDictionary<NSString *, NSString *> * _Nullable customStrings;
/// <code>livenessMode</code> is ignored (see the property) — it stays in the signature
/// so the ObjC selector doesn’t change.
...
/// Exposed to Objective-C for compatibility
SWIFT_CLASS("_TtC14VeryAILiveness10VeryResult")
...
SWIFT_CLASS("_TtC14VeryAILiveness17VeryScanPalmModel")
...
SWIFT_CLASS("_TtC14VeryAILiveness14VeryScanResult")
